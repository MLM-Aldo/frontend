...
const [rejectedWithdrawals, setRejectedWithdrawals] = useState([]);
  ...
  useEffect(() => {
    ...
    const approved = withdrawHistory.filter(item => item.amount_withdraw_status === 'Approved');
  ...
  const currentItems = approvedWithdrawals.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(approvedWithdrawals.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
