...
function ReferralUsers() {
  ...
  const getAllUsers = async (referralCode) => {
    try {
      const response = await axios.get(base_url + "users/allUsers");
      const allUsers = response.data.users;
      const filteredUsers = referralCode ? allUsers.filter(user => user.referredBy === referralCode) : allUsers;
      ...
      setFullUsers(filteredUsers);
      setUsers(filteredUsers);
      setCurrentPage(1);
      let totalItems = filteredUsers.length;
      let totalPages = Math.ceil(totalItems / 1); // 1 record per page
      let startIndex = (currentPage - 1) * 1;
      let endIndex = Math.min(startIndex + 1 - 1, totalItems - 1); // end index of current page
      const currentUsers = filteredUsers.slice(startIndex, endIndex + 1); // users to display in current page
